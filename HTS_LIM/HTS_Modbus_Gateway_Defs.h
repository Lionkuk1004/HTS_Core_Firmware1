...
namespace ProtectedEngine {
    ...
    inline bool Modbus_Is_Valid_State(Modbus_State s) noexcept
    {
        const uint8_t v = static_cast<uint8_t>(s);
        if (v == 0u) { return true; }
        if ((v & ~MODBUS_VALID_STATE_MASK) != 0u) { return false; }
        return ((v & (v - 1u)) == 0u);
    }

    inline bool Modbus_Is_Legal_Transition(Modbus_State from, Modbus_State to) noexcept
    {
        if (!Modbus_Is_Valid_State(to)) { return false; }
